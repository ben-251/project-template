...
price_fit_log10 <- price_wflow_10 %>% 
  fit(data = train_data)

laptop_test_pred_log10 <- predict(price_fit_log10,test_data) %>%
  bind_cols(test_data %>% select(Price))

rmse(laptop_test_pred_log10, truth = Price, estimate = .pred)
glance(price_fit_10)$adj.r.squared
```
...
price_fit_10_aug <- augment(model_object_1$fit)
...
price_fit_log10_aug <- augment(model_object_2$fit)
ggplot(price_fit_log10_aug, mapping = aes(x = .fitted, y = .resid)) +
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 0, color = "gray", lty = "dashed") +
  labs(x = "Predicted height", y = "Residuals")
```
