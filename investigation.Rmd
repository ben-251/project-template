...
laptop_data_final %>%
  select(Ram, Weight, Price, Ppi, HDD, SSD,storage_capacity,Inches,log_price) %>%
  summarise(
    across(
      .cols = everything(),  
      .fns = list(
...
#on to box plots now
variables <- c("Ram", "Weight", "Price", "Ppi", "HDD", "SSD","Log Price","Storage Capacity","Inches")

avg_stats_long <- laptop_data_final %>%
  select(TypeName, Ram, Weight, Price, Ppi, HDD, SSD, log_price, storage_capacity, Inches) %>% 
  pivot_longer(
    cols = Ram:Inches,          
    names_to = "Variable",   
    values_to = "Value"      
...
laptop_data_final <- laptop_data_final%>%
  mutate(GHz = str_extract(Cpu, "\\d+(\\.\\d+)?(?=GHz)"))
...
laptop_data_company %>%
  ggplot(mapping = aes(x=Inches,y=Price, color=Company)) +
  geom_smooth(se = FALSE, method = "lm")+
  theme_minimal() +
  labs(
    title = "Inches against Price",
...
#On the lecture slides it says you really only wanna use 2 predictors per model 
...
laptop_test_pred_1 <- predict(price_fit_1,test_data) %>%
...
#First Model is pretty terrible lol, I'm gonna add SSD
...
laptop_test_pred_2 <- predict(price_fit_2,test_data) %>%
  bind_cols(test_data %>% select(Price))

rsq(laptop_test_pred_2, truth = Price, estimate = .pred, adj = TRUE)
rmse(laptop_test_pred_2, truth = Price, estimate = .pred)

# A little bit better, predictions are off by like 200 pounds
