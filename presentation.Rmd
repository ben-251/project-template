...
- The variables with highest correlation are Ram and Ssd
- The highest difference in intercept is 0.65 for computer type
...
=======
>>>>>>> 25368a1423690cfca21c0a392bbdeea0077199a8
class: center, middle
...
table <- tidy(price_log10_fit2$fit) %>%
  ...
  mutate(
    estimate = round(estimate, 4),
    exp_estimate = round(exp(estimate),4)
  ) %>%
...
$$
\widehat{logPrice} = 5.5929 +  (0.0271 \times Ram) + (0.0009 \times SSD) + \dots
$$
---
## Numeric Variables
For every additional GB of Ram,all else held constant, the estimated price increases on average by a factor of 1.0275

For every additional GB of SSD, all else held constant, the estimated price increases on average by a factor of 1.0009

## Categorical
Each category also has a baseline:

The base line intercept has no meaning as a laptop cannot have 0 RAM, and a laptop cannot have "no company" or "no type of laptop"

For:
- The price increases by a factor of 1.1042 for Gaming Laptops relative to 2-in-1 convertible laptops
- The price decreases by a factor of 0.8492 for Notebooks relative to 2-in-1 Convertible laptops
